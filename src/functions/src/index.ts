...
export const onInvoiceCreated = onDocumentCreated("salesInvoices/{invoiceId}",
  async (event) => {
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        if (invoice.amountPaid > 0) {
            ...
            transaction.set(advanceJvRef, {
                id: advanceJvRef.id,
                date: invoice.date,
                narration: `Adjustment of advance for Invoice ${invoice.invoiceNumber}`,
                entries: [
                    { accountId: customerLedgerId, debit: 0, credit: invoice.amountPaid },
    ...
    } catch (e) { console.error("Invoice JV creation failed: ", e); }
  });
