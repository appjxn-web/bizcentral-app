...
export default function SalesInvoiceDetailPage() {
  ...
  const actorUid = "CURRENT_USER_UID";
  ...
  // Optional previews
  const [journalLines, setJournalLines] = React.useState<any[]>([]);
  ...
  async function loadPreviews() {
    if (!invoice?.voucherId) return;
    setPreviewLoading(true);
    try {
      // journal entries (read-only allowed in rules)
      const jRef = collection(firestore, `companies/${companyId}/journal_entries`);
      const jQ = query(jRef, where("voucherId", "==", invoice.voucherId), orderBy("lineNo", "asc"), limit(50));
      const jSnap = await getDocs(jQ);
      setJournalLines(jSnap.docs.map((d) => ({ id: d.id, ...d.data() })));

      // stock movements for this invoice
      const mRef = collection(firestore, `companies/${companyId}/stock_movements`);
      const mQ = query(mRef, where("refType", "==", "SALES_INVOICE"), where("refId", "==", invoiceId), limit(200));
  ...
  React.useEffect(() => {
    // Auto load previews after posted
  ...
  }, [invoice?.status, invoice?.voucherId]);
          ...
          {invoice && (
            <>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {status === "POSTED" && (
                  <Button variant="secondary" onClick={loadPreviews} disabled={previewLoading}>
                    {previewLoading ? "Loading..." : "Reload Posting Preview"}
                  </Button>
                )}
              </div>
            </>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-2 text-sm">
              {journalLines.length === 0 && <div className="text-muted-foreground">No journal lines found.</div>}
              {journalLines.map((l) => (
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-2 text-sm">
              {movements.length === 0 && <div className="text-muted-foreground">No stock movements found.</div>}
              {movements.map((m) => (
