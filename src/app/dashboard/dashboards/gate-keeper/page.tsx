...
import { gateLog as initialGateLog } from '@/lib/data';
...
function GateKeeperPageContent() {
  ...
  const [log, setLog] = React.useState<GateEntry[]>(initialGateLog);
  ...
  const handleSubmit = () => {
    ...
    if (entryType === 'Inward') {
      ...
      toast({ title: 'Inward Entry Logged', description: `${personName} has been logged in.` });
        ...
        if (entryToUpdate) {
            setLog(prev => prev.map(entry => entry.id === entryToUpdate.id ? { ...entry, outTime: new Date().toISOString() } : entry));
  ...
  const entriesToday = log.filter(e => e.inTime.startsWith(today)).length;
  const exitsToday = log.filter(e => e.outTime && e.outTime.startsWith(today)).length;
