...
import type { ServiceRequest, CompanyInfo } from '@/lib/types';
...
export default function ServiceInvoiceViewPage() {
    ...
    React.useEffect(() => {
        const data = localStorage.getItem('serviceInvoiceData');
        if (data) {
            const parsedData = JSON.parse(data);
            if (parsedData.id === serviceRequestId) {
    ...
    const calculations = React.useMemo(() => {
        if (!invoiceData?.quotation?.items) return { subtotal: 0, totalGst: 0, grandTotal: 0 };
        const items = invoiceData.quotation.items;
        const subtotal = items.reduce((acc: number, item: any) => acc + (item.quantity * item.rate), 0);
        const totalGst = items.reduce((acc: number, item: any) => acc + (item.quantity * item.rate * (item.gstRate / 100)), 0);
    ...
    const handleDownloadPdf = async () => {
        ...
        pdf.save(`ServiceInvoice-${invoiceData.id}.pdf`);
        setIsDownloading(false);
    };

    if (!serviceRequestId) {
        return <PageHeader title="Service Invoice Not Found" />;
    }
    
    if (!invoiceData || companyInfoLoading) {
        return (
             <PageHeader title="Loading Service Invoice...">
    ...
    return (
        <>
            <PageHeader title={`Service Invoice: ${serviceRequestId}`}>
                    ...
                    <div className="max-w-4xl mx-auto p-8" ref={pdfRef}>
                        <header className="flex justify-between items-start border-b pb-4">
                            ...
                            <div className="text-right">
                                <h1 className="text-2xl font-bold text-primary">Service Invoice</h1>
                        ...
                        <section>
                            <Table>
                                ...
                                <TableBody>
                                    {invoiceData.quotation?.items.map((item: any, index: number) => (
                                        <TableRow key={index}>
                                ...
                                <TableFooter>
                                    ...
                                    <TableRow>
                                        ...
                                        <TableCell className="text-right">{formatIndianCurrency(calculations.totalGst)}</TableCell>
                                    </TableRow>
