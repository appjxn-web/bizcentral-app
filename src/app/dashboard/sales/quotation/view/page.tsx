...
import type { Quotation, CompanyInfo, Party, CoaLedger, Address, UserProfile } from '@/lib/types';
...
export default function QuotationViewPage() {
    ...
    const { data: creatorData, loading: creatorLoading } = useDoc<UserProfile>(
        (quotationData as any)?.createdByUid ? doc(firestore, 'users', (quotationData as any).createdByUid) : null
    );
    ...
    const isLoading = quotationLoading || customerLoading || companyInfoLoading || bankLedgerLoading || creatorLoading;
      ...
      <Card className="p-8 print-container mx-auto max-w-4xl">
        ...
        <header className="flex justify-between items-start border-b pb-4">
            ...
            <div className="text-right">
                ...
                <p className="text-sm text-muted-foreground">{companyAddress?.line1}, {companyAddress?.line2}</p>
                <p className="text-sm text-muted-foreground">{companyAddress?.city} - {companyAddress?.pin}, {companyAddress?.district}, {companyAddress?.state}, {companyAddress?.country}</p>
                <p className="text-sm text-muted-foreground">{companyInfo?.contactEmail} | {companyInfo?.contactNumber} | {companyInfo?.website}</p>
        ...
        <section className="my-6">
            ...
            <div className="flex justify-between">
                <div>
                    ...
                    <p>{customerData?.address?.line1}, {customerData?.address?.line2}</p>
                    <p>{customerData?.address?.city} - {customerData?.address?.pin}, {customerData?.address?.district}, {customerData?.address?.state}, {customerData?.address?.country}</p>
                    <p>{customerData?.contactPerson && `Attn: ${customerData.contactPerson}, `}{customerData?.email} | {customerData?.phone}</p>
                    <p>GSTIN: {customerData?.gstin || 'N/A'}</p>
                </div>
                <div className="text-right">
        ...
        <section>
          <Table>
            ...
            <TableFooter>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-right font-bold">Taxable Value</TableCell>
                  <TableCell className="text-right font-bold">{formatIndianCurrency(calculations.taxableAmount)}</TableCell>
                </TableRow>
                {isInterstate ? (
                    <TableRow>
        ...
        <footer className="flex justify-between items-end mt-16">
            <div className="text-xs space-y-4">
                <div>
                    ...
                    <div className="text-muted-foreground min-h-[4.5rem] whitespace-pre-wrap">{quotationData.terms}</div>
                </div>
                 <div className="flex gap-4">
                    {bankDetails && (
                        <div className="text-xs">
                            <h4 className="font-bold mb-1">Bank Details for Booking Payment ({formatIndianCurrency(advanceAmount)}):</h4>
                    ...
                    {companyInfo?.primaryUpiId && advanceAmount > 0 && (
                        <div className="flex flex-col items-center">
                            <h4 className="font-bold text-xs mb-1">Scan to Pay:</h4>
                            <QRCodeSVG value={`upi://pay?pa=${companyInfo.primaryUpiId}&pn=${encodeURIComponent(companyInfo.companyName || '')}&am=${advanceAmount.toFixed(2)}&cu=INR`} size={80} />
                        </div>
                    )}
                </div>
            </div>
            <div className="text-right">
                ...
                <Separator />
                <p className="text-sm pt-1">Authorized Signatory ({creatorName})</p>
            </div>
        </footer>
      </Card>
    </>
